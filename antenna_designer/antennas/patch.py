...
def _cst_unit(ctx: Context) -> str:
    return "mil" if ctx.unit_str == "mils" else "mm"


def _cst_store_params(params: dict[str, float]) -> list[str]:
    lines = []
    for name, value in params.items():
        lines.append(f'    StoreDoubleParameter "{name}", {value:.12g}')
    return lines


def _cst_material_block(material_name: str = "Antenna_Substrate") -> list[str]:
    return [
        "With Material",
        "     .Reset",
...
def _cst_brick(name: str, component: str, material: str,
               xr: tuple[str, str], yr: tuple[str, str],
               zr: tuple[str, str]) -> list[str]:
    return [
        "With Brick",
        "     .Reset",
        f'     .Name "{name}"',
...
def _cst_cylinder(name: str, component: str, material: str, *,
                  outer_radius: str, inner_radius: str = "0",
                  axis: str = "z", zrange: tuple[str, str],
                  xcenter: str = "0", ycenter: str = "0",
                  segments: str = "0") -> list[str]:
    return [
        "With Cylinder",
        "     .Reset",
        f'     .Name "{name}"',
...
def _rect_patch_rear_cst_macro(ctx: Context, r: dict) -> str:
    """Whole-model CST macro for the rear-fed rectangular patch.

    This mirrors matlab_scripts/design_scripts/rearFedPatchCSTConnect.m, but
    is emitted directly from the GUI and keeps every CST primitive tied to
    ...
    params = {
        ...
        "W": r["W"] * m,
        "L": r["L"] * m,
        "dielectric_t": ctx.h * m,
        "top_cu_t": ctx.cu_thickness * m,
        "bot_cu_t": ctx.cu_thickness * m,
        "Ls": ctx.Ls * m,
        "Ws": ctx.Ws * m,
        "x_feed": r["x_feed"] * m,
        "pin_rad": r["pin_rad"] * m,
        "sma_rad": r["sma_rad"] * m,
        "pin_ext": r.get("pin_ext", 0.0) * m,
    }
    lines = [
    ...
    lines.extend(_cst_store_params(params))
    lines.extend([
        "",
        "    Solver.FrequencyRange \"fr_GHz*0.75\", \"fr_GHz*1.25\"",
        "",
    ])
    lines.extend(_cst_material_block())
    lines.append("")
    lines.extend(_cst_brick(
        "Substrate", "Antenna", "Antenna_Substrate",
        ("-L/2 - Ls", "L/2 + Ls"),
        ("-W/2 - Ws", "W/2 + Ws"),
        ("bot_cu_t", "bot_cu_t + dielectric_t")))
    lines.append("")
    lines.extend(_cst_brick(
        "Ground", "Antenna", "PEC",
        ("-L/2 - Ls", "L/2 + Ls"),
    ...
    lines.extend(_cst_brick(
        ...
        ("-L/2", "L/2"),
        ("-W/2", "W/2"),
        ("bot_cu_t + dielectric_t",
         "bot_cu_t + dielectric_t + top_cu_t")))
    lines.append("")
    lines.extend(_cst_cylinder(
        "Feed_Pin", "Antenna", "PEC",
        outer_radius="pin_rad",
        zrange=("0", "bot_cu_t + dielectric_t + top_cu_t + pin_ext"),
        xcenter="x_feed", ycenter="0"))
    lines.append("")
    lines.extend(_cst_cylinder(
        "Coax_Dielectric_Plug", "Antenna", "Antenna_Substrate",
        outer_radius="sma_rad", inner_radius="pin_rad",
        zrange=("0", "bot_cu_t"), xcenter="x_feed", ycenter="0"))
    lines.extend([
        "",
...
def _rect_patch_inset_cst_macro(ctx: Context, r: dict) -> str:
    """Whole-model CST macro for the inset edge-fed rectangular patch."""
    m = ctx.out_mult
    params = {
        "fr_GHz": ctx.fr / 1e9,
        "er": ctx.er,
        ...
        "W_feed": r["W_feed"] * m,
        "y0": r["y0"] * m,
        "inset_gap": r["inset_gap"] * m,
        "port_margin": 1.5 * r["W_feed"] * m,
    }
    top_z0 = "bot_cu_t + dielectric_t"
    top_z1 = "bot_cu_t + dielectric_t + top_cu_t"
    lines = [
        "' Full tunable CST model: Rectangular Patch - Inset Edge-Fed",
        "' Generated by Antenna Designer",
        "' Edit the Parameter List in CST, then Rebuild to retune dimensions.",
        "",
        "Sub Main ()",
    ...
    lines.extend(_cst_store_params(params))
    lines.extend(["", "    Solver.FrequencyRange \"fr_GHz*0.75\", \"fr_GHz*1.25\"", ""])
    lines.extend(_cst_material_block())
    lines.append("")
    lines.extend(_cst_brick(
        "Substrate", "Antenna", "Antenna_Substrate",
    ...
    lines.append("")
    lines.extend(_cst_brick(
        "Top_Copper", "Antenna", "PEC",
        ("-L/2", "L/2"),
        ("-W/2", "W/2"),
        (top_z0, top_z1)))
    lines.append("")
    lines.extend(_cst_brick(
        "Feed_Line", "Antenna", "PEC",
        ("L/2 - y0", "L/2 + Ls"),
        ("-W_feed/2", "W_feed/2"),
        (top_z0, top_z1)))
    lines.extend([
        "",
        "    Solid.Add \"Antenna:Top_Copper\", \"Antenna:Feed_Line\"",
        "",
    ])
    for name, yr in (
        ("Inset_Gap_Top", ("W_feed/2", "W_feed/2 + inset_gap")),
        ("Inset_Gap_Bottom", ("-W_feed/2 - inset_gap", "-W_feed/2")),
    ):
        lines.extend(_cst_brick(
            name, "Antenna", "Vacuum",
            ("L/2 - y0", "L/2"),
            yr,
            (top_z0, top_z1)))
        lines.append(f'    Solid.Subtract "Antenna:Top_Copper", "Antenna:{name}"')
        lines.append("")
    lines.extend([
        "    On Error Resume Next",
        "    Port.Delete \"1\"",
        ...
        "         .Yrange \"-W_feed/2\", \"W_feed/2\"",
        "         .Zrange \"0\", \"bot_cu_t + dielectric_t + top_cu_t\"",
        "         .XrangeAdd \"0\", \"0\"",
        "         .YrangeAdd \"0\", \"0\"",
        "         .ZrangeAdd \"0\", \"0\"",
        "         .SingleEnded \"False\"",
...
def _circular_patch_cst_macro(ctx: Context, r: dict) -> str:
    """Whole-model CST macro for a rear-fed circular patch."""
    m = ctx.out_mult
    params = {
        "fr_GHz": ctx.fr / 1e9,
        "er": ctx.er,
    ...
    lines = [
        "' Full tunable CST model: Circular Patch - Rear-Fed",
        "' Generated by Antenna Designer",
        "' Edit the Parameter List in CST, then Rebuild to retune dimensions.",
        "",
        "Sub Main ()",
...
class RectPatchInset(AntennaBase):
    ...
    def compute(self, ctx: Context, params: dict) -> dict:
        ...
        results["cst_full_model_macro"] = _rect_patch_inset_cst_macro(ctx, results)
        return results
...
class RectPatchRear(AntennaBase):
    ...
    def compute(self, ctx, params):
        ...
        pin_rad = pin_dia_m / 2
        pin_ext = ctx.m(params.get("pin_ext", "0.0"))
        ...
        results = {
            ...
            "Z_coax": Z_coax, "sma_er": sma_er, "pin_ext": pin_ext,
            "L_eff": L + 2 * dims["dL"],
            "board_size": (L + 2 * ctx.Ls, W + 2 * ctx.Ws),
        }
        results["cst_full_model_macro"] = _rect_patch_rear_cst_macro(ctx, results)
        return results
...
class CircularPatch(AntennaBase):
    ...
    def compute(self, ctx, params):
        ...
        results["cst_full_model_macro"] = _circular_patch_cst_macro(ctx, results)
        return results
