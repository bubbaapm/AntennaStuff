...
// ==================================================
// g3990200: With CSP is in place, inline JS does not execute in web doc
// The following event listeners hook up event attributes to relevant HTML to workaround CSP

document.addEventListener("DOMContentLoaded", () => {
    const toggleElements = document.querySelectorAll('.toggleExpander');
    toggleElements.forEach((toggleElt) => {
        toggleElt.addEventListener("click", (e) => {
            e.preventDefault();
            const { blockId, arrowId } = toggleElt.dataset;
            toggleexpander(blockId, arrowId);
        })
    });
});
...
window.addEventListener('load', function () {
    // If the URL contains consecutive slashes, except those immediately following http: or https:,
    // replace with a single slash. See g3010139.
    const url = window.location.href;
    const consecutiveSlashesRegex = /(?<!https?:)\/{2,}/g;
    if (consecutiveSlashesRegex.test(url)) {
        const updatedURL = url.replace(consecutiveSlashesRegex, '/');
        if (updatedURL !== url) {
    ...
    let targetID = '#nav_scrollspy'; // Default target ID
...
$(document).ready(function () {
    ...
    $('.collapse').on('hidden.bs.collapse shown.bs.collapse', function () {
        var id = $(this).attr("id");
        var expandParent = $("[data-bs-target='#" + id +"']");
    ...
    $('#doc_center_content .expandAllLink').click(function (e) {
        e.stopPropagation();
        var link = $(this);
        var allExpanders = link.closest('.expandableContent').find("[data-bs-toggle='collapse']");
    ...
    $('#expandAllPage').click(function () {
        ...
        let allAccordionExpanders = $("#doc_center_content").find("[data-bs-toggle='collapse']");
        
        if ($(this).data('allexpanded')) {
            doCollapseAllInPage(allAccordionExpanders);
            $(this).data('allexpanded', false);
            $(this).html(getLocalizedString('expand_all_in_page'));
        } else {
            doExpandAllInPage(allAccordionExpanders);
            $(this).data('allexpanded', true);
            $(this).html(getLocalizedString('collapse_all_in_page'));
        }

        // Handle Toggle Expanders 
        let allToggleExpanders = document.querySelectorAll('.expander');
        let arrows = document.querySelectorAll('img[id*="_expandable_text"]');

        allToggleExpanders.forEach((block, index) => {
        ...
        return false; // Prevent default link behavior/propagation
    ...
    $(window).bind('toc_resize', function () {
        $('#search_crumb_container').width($('#doc_center_content').width());
...
function applySearchHighlight() {
    if ($.fn.highlight) {
        ...
        // Use the DOMParser method to parse a string into a DOM Document to decode any HTML entities in the highlight term.
        const searchHighlightTermDecoded = decodeHtmlEntities(searchHighlightTerm);
        searchHighlightTerm = searchHighlightTermDecoded;
        ...
        if (searchHighlightTerm.length > 0) {
            ...
            // When coming from search, focus on h1 but don't scroll to it.
...
/**
 * Redirects the current page by replacing the value of the 'category' query parameter
 * with 'index'. The function captures the current URL, identifies the 'category' parameter,
 * and modifies its value, then redirects the browser to the updated URL.
 */
function redirectToproduct() {
    var currentURL = window.location.href;
    var regex = new RegExp('([?&]category=)([^&]*)', 'i');
    var updatedURL = currentURL.replace(regex, function(match, p1) {
        return p1 + 'index';
    });
...
function expandCollapsedContent() {
    if (location.hash.length > 0) {
        ...
        if (target.length > 0) {
            let targetNoPanel = target.parents("[class='expand_collapse']").children("[data-bs-toggle='collapse']");
            if (targetNoPanel.length > 0) {
                target = targetNoPanel;
            }
            var expandParent = getExpandParentForAnchorTarget(target);
            prepareEltForExpansion(expandParent);
            var expandAncestors = target.parents("div.panel").children("[data-bs-toggle='collapse']");
...
function getExpandParentForAnchorTarget(target) {
    var collapseToggle = target.parents("[data-bs-toggle='collapse']");
...
function smoothScrollEventCallback(evt,hash,target) {
    ...
    var expandParent = getExpandParentForAnchorTarget(target);
    prepareEltForExpansion(expandParent);
    let accordionContainerAncestor = target.parents("div.mw_ws_accordion_container").eq(1);
    if (accordionContainerAncestor.length) {
        let firstAccordionButton = accordionContainerAncestor.find('button[data-bs-toggle="collapse"]').first();
        let buttons = [expandParent, firstAccordionButton];
    ...
    var scrollTop = target.offset().top - getScrollTopAdjustment();
    $('.anchor_hinting').removeClass('anchor_hinting');
    $(scrollParameter).animate({scrollTop: scrollTop}, 0, function () {
        target.addClass('anchor_hinting');
        setTimeout(function () {
            target.removeClass('anchor_hinting');
        }, 5000);
...
function addSmoothScroll() {
    ...
    // SVG in <object>
    $('object[type="image/svg+xml"]').on('load', function () {
        let svgDoc = this.contentDocument;
        let svgLinks = svgDoc.querySelectorAll('a');
        svgLinks.forEach(function (link) {
            link.addEventListener('click', function (evt) {
                // Get the xlink:href attribute (namespaced)
                const xlinkNS = "http://www.w3.org/1999/xlink";
                let href = this.getAttributeNS(xlinkNS, "href");
                // Or, for some browsers, try getAttribute('xlink:href')
                if (!href) href = this.getAttribute('xlink:href');

                if (href && href.indexOf('#') !== -1) {
                    let hash = href.substring(href.indexOf('#'));
                    let target = getInternalLinkTarget(hash);
                    if (target.length > 0) {
                        smoothScrollEventCallback(evt, hash, target);
                    }
                }
...
// Assumes that the elt passed in has data-bs-toggle=collapse, and
// data-bs-target specifying the element which needs to be expanded/collapsed
function findExpandableContent(elt) {
    var target = elt.data('bs-target');
    return $(target);
}
...
function checkExpandAllLinkState(elt) {
    ...
    var expandableChildren = expandableParent.find("[data-bs-toggle='collapse']");
...
function doExpandNestedParent(elt, noAnimation) {
    var expandableParent = elt.parents("[data-bs-toggle='collapse']").first();
...
function doExpandAllInPage(elt_array) {
    ...
    $.each($('#doc_center_content .expandAllLink'), function (i, link) {
        setExpandAllLinkState($(link), "expanded");
    });
}
...
/**
 * Calculates the scroll top adjustment based on the current document center type and screen width.
 * This is used to offset scrolling so that sticky headers and navigation bars do not cover the target content.
 * 
 * @returns {number} The total height in pixels to adjust scroll position by.
 */
function getScrollTopAdjustment() {
    let scrollTop = 0;
    let localNavHeight = 0;

    if (typeof getDocCenterType === 'function') {
        const docCenterType = getDocCenterType();

        if (docCenterType === 'product') {
            // For installed doc: sticky header container is "Documentation", "Examples etc" tabs which is only sticky on larger screens
            // Section header is the top Help Center section header + Search box and is always sticky
            if (window.screen.width > 767) {
                localNavHeight = $('.sticky_header_container').height() || 0;
            }
            scrollTop = (localNavHeight + ($('.section_header').height() || 0) + 10);
        } else {
            // For web doc: navbar container is MATLAB Help Center + Community + Learning nav which is always sticky
            // Localnav is Documentation, Examples, etc. tabs which is only sticky on larger screens
            if (window.screen.width >= 992) {
                localNavHeight = $('.localnav').height() || 0;
            }
            scrollTop = (($('.navbar').height() || 0) + localNavHeight + 10);
        }
    }
    return scrollTop;
}
...
function refreshScrollSpy() {
    try {
      const bodyElement = document.querySelector('body#responsive_offcanvas');
      if (bodyElement) {
        const scrollSpyInstance = bootstrap.ScrollSpy.getInstance(bodyElement);
        if (scrollSpyInstance) {
          scrollSpyInstance._config.offset = getScrollTopAdjustment() + 1;
          scrollSpyInstance.refresh();
        }
      }
      // Trigger a scroll event to update the ScrollSpy state
...
// Handle anchor links that point to content within accordion content bodies
document.addEventListener('DOMContentLoaded', function() {
    const hash = window.location.hash;
    if (hash) {
        const anchorTarget = document.querySelector(hash);
        if (anchorTarget) {
            // Check if the anchorTarget is within an accordion structure
            let inAccordion = anchorTarget.closest('.accordion-collapse');
            while (inAccordion) {
                // Check if the accordion is already expanded
                if (!inAccordion.classList.contains('show')) {
                    new bootstrap.Collapse(inAccordion, {
                        toggle: true
                    });
                }
                inAccordion = inAccordion.parentElement.closest('.accordion-collapse');
            }
        }
    }
});

// searchHighlight Decoder. Duplicated in releasenotes.js as decodeHtmlEntitiesRN.
...
// Copyright 2002-2025 The MathWorks, Inc.
